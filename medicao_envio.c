...
#define ECHO_TIMEOUT_US                                                        \
  30000               // tempo máximo para esperar o echo (em microssegundos)
...
volatile bool cancelar_coleta =
    false; // flag de cancelamento de coleta pelo botão B

int menu = 0;
bool conectar = true;

float calibrador = 78.01688f; // altura do sensor em relacao ao fundo (cm) para
                              // obter 550L com sensor a 20cm da tampa
...
static struct mqtt_connect_client_info_t mqtt_ci = {.client_id = "pico_balanca",
                                                    .keep_alive = 30};
...
void iniciar_conexao_mqtt(void) {
  ...
  err_t err = mqtt_client_connect(client, &mqtt_server_ip, MQTT_BROKER_PORT,
                                  mqtt_connection_cb, NULL, &mqtt_ci);
...
void setup_sensor(uint sensor_pin) {
  ...
  gpio_set_irq_enabled_with_callback(sensor_pin, GPIO_IRQ_EDGE_FALL, true,
                                     &flow_sensor_isr);
}

void medir_volume(uint sensor_pin, uint32_t pulse_count, bool *last_reg,
                  float *contador_volume, bool *enviar) {
  ...
  if (!(*enviar)) {
    printf("Sensor: %u | Pulsos: %lu | Vazao: %.2f L/min | Volume: %.2fL | "
           "Tempo: %ds | %d\n ",
...
void mqtt_connection_cb(mqtt_client_t *client, void *arg,
  ...
  if (status == MQTT_CONNECT_ACCEPTED) {
    ...
    ssd1306_WriteString("MQTT Conectado!", Font_7x10, White);
    ssd1306_UpdateScreen();
    ...
    ssd1306_WriteString("Erro no MQTT", Font_7x10, White);
    ssd1306_UpdateScreen();
    sleep_ms(1500);
...
void publish_peso(float vol2) {
  ...
  int len =
      snprintf(msg, sizeof(msg), "{\"id\":%s,\"peso\":%.1f}", nome_atual, peso);
...
float volumeLeite(float distancia) {
  ...
  // Para que o tanque cheio de 550 L (550.000 cm³) corresponda a uma distância
  // medida de 20.0 cm (sensor a 20cm da tampa/superfície do leite), a altura
  // total do sensor (calibrador) deve ser: H_leite + 20.0 = (550.000 / (79.0 *
  // 120.0)) + 20.0 = 58.0168776 + 20.0 = 78.0168776 cm.
...
int main() {
  ...
  add_repeating_timer_ms(INTERVALO_CALLBACK, monitor_buttons, NULL,
                         &timer); // timer que verifica os botões pressionados
  ...
  ssd1306_WriteString("Conectando WiFi...", Font_6x8, White);
  ssd1306_UpdateScreen();
  ...
  if (cyw43_arch_wifi_connect_timeout_ms(WIFI_SSID, WIFI_PASSWORD,
    ...
    ssd1306_WriteString("Erro no WiFi!", Font_7x10, White);
    ssd1306_UpdateScreen();
    while (1)
      ;
  }
  ...
  ssd1306_WriteString("WiFi Conectado!", Font_7x10, White);
  ssd1306_SetCursor((uint8_t)(64 - ((strlen(WIFI_SSID) * 6) / 2)), 36);
  ssd1306_WriteString(WIFI_SSID, Font_6x8, White);
  ssd1306_UpdateScreen();
  sleep_ms(1500);
  ...
  ssd1306_DrawRectangle(0, 0, 127, 63, 1);
  ssd1306_SetCursor(10, 28);
  ssd1306_WriteString("Conectando MQTT...", Font_6x8, White);
  ssd1306_UpdateScreen();
  ...
  while (true) {
    ...
    if (!mqtt_connected &&
        absolute_time_diff_us(last_mqtt_reconnect_attempt,
      ...
      ssd1306_WriteString("Reconectando MQTT...", Font_6x8, White);
      ssd1306_UpdateScreen();
    ...
    if (absolute_time_diff_us(last_status_log, get_absolute_time()) > 2000000) {
      ...
      printf("ESTADO mqtt=%d conectando=%d aguardando=%d pendente_envio=%d\n",
             mqtt_connected, mqtt_connecting, aguardando_peso,
             !peso_ja_enviado);
    }
      ...
      if (mqtt_connected && aguardando_peso && !peso_ja_enviado) {
        if (BT_B_state) {
          ...
          printf("Distancia: %.2f cm | Media: %.2f cm | Volume: %.2f L\n",
                 distancia, distancia_suavizada, volume_atual_cm3 / 1000.0f);
        ...
        if (envia1 && BT_B_state && confirmar) {
          if (conectar) {
            if (mqtt_connected && aguardando_peso && !peso_ja_enviado &&
                absolute_time_diff_us(last_publish, get_absolute_time()) >
                    1000000) {
              last_publish = get_absolute_time();
        ...
        if (mqtt_connected) {
          ...
          ssd1306_WriteString("Sem Conexao", Font_7x10, White);
          ssd1306_UpdateScreen();
        }
        sleep_ms(200);
      }
